...
import { getInsideById , putInside} from '../services/Inside';
  ...
  useEffect(() => {
    const fetchInside = async () => {
      try {
        ...
        console.error('Error al obtener el interior:', error);
      }
    };
 ...
 const handleImageSelection = (imageId) => {
  ...
  const handleConfirm = () => {
    if (selectedImages.length > 0) {
      ...
      putPromises.push(putInsidePromise);
          ...
          <Stack>
            ...
            <Group>
              ...
              <Modal opened={opened} onClose={handleCancelModal} size={'100%'}>
                ...
                <Modal.Body>
                  <Group>
                    {inside.Images && inside.Images.map((image, index) => (
                      <Stack key={index}>
                        <Checkbox
